...
Earlier this month I migrated three production AKS clusters off the community `ingress-nginx` controller and onto the F5 NGINX Ingress Controller OSS (v2.5.1). The apps were a compliance API platform (TravelRule), a real-time WebSocket trading server (LightStream), and a charting frontend (LightChart). Same controller name, completely different internals — and enough sharp edges to fill a post.
...
The full values for the base controller config across all three clusters ended up looking like this:
...
**`telemetryReporting.enable: false`** — F5 attempts to phone home to `oss.edge.df.f5.com`. In a cluster with no outbound internet on the node pool, this causes the controller pod to enter a crash-loop on startup waiting for the connection to timeout. Must be disabled.
...
```yaml
# community annotations (application ingress manifests)
...
controller:
  config:
    entries:
      http-snippets: |
        geo $lightchart_limit_bypass {
          default 0;
          150.249.232.224/28 1;  # office ranges bypass rate limits
          203.114.50.208/28  1;
        }

        map $lightchart_limit_bypass $lightchart_limit_key {
          0 $binary_remote_addr;
          1 "";
        }

        limit_req_zone  $lightchart_limit_key zone=lightchart_rpm:10m rate=120r/m;
        limit_conn_zone $lightchart_limit_key zone=lightchart_conn:10m;
```
...
annotations:
  nginx.org/server-snippets: |
    limit_req zone=lightchart_rpm burst=80 nodelay;
    limit_req_status 429;
    limit_conn lightchart_conn 60;
    limit_conn_status 429;
```

The geo+map pattern lets specific IP ranges (office networks, CI runners, load testing IPs) bypass rate limits by mapping to an empty key — which `limit_req_zone` treats as unlimited. This is cleaner than maintaining allow-lists in multiple places.

---

## LightStream: WebSocket Keepalive Surprises

LightStream is a Socket.io server behind WebSocket connections. Everything looked healthy post-migration — pods up, ingress adopted — but Socket.io clients started disconnecting every 30–60 seconds.

The root cause: F5's default `keepalive-timeout` is `0s` (disabled), whereas the community chart defaults to `60s`. WebSocket connections through NGINX depend on the underlying TCP keepalive to stay alive during idle periods. With keepalive disabled, NGINX was closing the connection on the server side.
...
Also added the F5 WebSocket annotation to the ingress manifest:

```yaml
annotations:
  nginx.org/websocket-services: "lightstream-io,lightstream-ws"
```

Without this annotation, F5 does not set the necessary `Upgrade` and `Connection` proxy headers for WebSocket handshakes.

---

## LightStream: Zero-Downtime Service Selector Patch

LightStream runs a secondary service `horizon-b-ingress-nginx-controller` that routes specific traffic. Its label selector was hardcoded to the community labels:
...
I couldn't just `kubectl apply` a new selector because Kubernetes rejects selector changes on existing Services. Instead, I patched it as part of the pipeline pre-upgrade step:

```bash
kubectl patch service horizon-b-ingress-nginx-controller \
  -n nginx-ingress \
  --type='merge' \
  -p '{
    "spec": {
...
The `--type='merge'` patch strategy replaces only the keys specified, leaving the rest of the selector intact. This ran before `helm upgrade` so the service selector matched the new pods the moment they came up.

---

## TravelRule: cert-manager and Datadog
...
Also removed a cert-manager feature gate that was only needed to work around a community ingress-nginx bug (issue #11176) related to path type handling. F5 doesn't have the bug:
...
F5 exposes Prometheus metrics on port `9113` (community used `8080`). The existing Datadog auto-discovery configuration was pointing at the wrong port and namespace. I added an OpenMetrics check:
...
If your node pool doesn't carry the `beta.kubernetes.io/os` label (newer AKS node images dropped it), the community controller pods won't schedule at all. This isn't migration-specific but it's a good cleanup to land at the same time.

---

## Helm Upgrade Stability

On cold nodes (newly scaled-up node pool), the F5 controller image pull can take longer than Helm's default 3m timeout. `--wait --timeout 5m` avoids spurious pipeline failures that previously looked like deployment regressions but were actually just slow image pulls:
...
Here is the honest account of what broke and when:

| Time | Cluster | Issue | Fix |
|------|---------|-------|-----|
| T+0 | TravelRule | F5 crash-loops on startup | `telemetryReporting.enable: false` + `enableCustomResources: false` |
| T+0 | TravelRule | Linkerd not injecting pods | Fixed annotation path: `podAnnotations` → `controller.pod.annotations` |
| T+0 | TravelRule | Datadog scraping wrong port | Added OpenMetrics check on port 9113 |
| T+0 | TravelRule | Datadog system-probe seccomp failures | `systemProbe.enabled: false`, `discovery.enabled: false` |
| T+1h | LightStream | All LB backends unhealthy | Switched Azure LB probe from HTTP `/healthz` to TCP |
| T+2h | LightStream | Socket.io disconnections | `keepalive-timeout: 60s`, `nginx.org/websocket-services` annotation |
| T+3h | LightStream | `horizon-b` service endpoints empty | Pre-upgrade service selector patch |
| T+24h | LightStream | Helm timeout on cold nodes | `--wait --timeout 5m` |
| T+10d | TravelRule | IngressClass delete too aggressive | Made delete conditional on `spec.controller` value |

The conditional IngressClass delete came last because the initial unconditional delete worked fine on the first run — the risk only became apparent when thinking about pipeline reruns.
...
|------|------------------------|-------------|
| Helm repo | `kubernetes.github.io/ingress-nginx` | OCI: `ghcr.io/nginx/charts/nginx-ingress` |
...
| WebSocket | Automatic via annotation | `nginx.org/websocket-services` required |
...
**Audit every config key before migrating.** I spent more time than necessary discovering that F5 silently ignores unknown config keys. A pre-migration diff against the F5 config reference would have caught the `upstream-keepalive-*` and `use-gzip` removals before they hit production.

**Test WebSocket apps on a staging cluster first.** The Socket.io keepalive issue was predictable — I just didn't think to check default timeout differences between the two controllers.

**Add the service selector patch to the playbook from the start.** The `horizon-b` service issue in LightStream was specific to that app, but the general pattern of services with hardcoded community labels is common. It's worth grepping for `ingress-nginx` in all Service selectors before starting.
